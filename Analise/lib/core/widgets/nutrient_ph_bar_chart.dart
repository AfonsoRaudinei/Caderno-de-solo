...
class NutrientPhBarChart extends StatelessWidget {
  ...
  List<_NutrientBarData> _interpolate(double rawPh) {
    ...
    if (phValues.contains(safePH)) {
      return _nutrientGroups
          .map((g) => _NutrientBarData(
                label: g.label,
            ...
            children: [
              ...
              Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border:
                      Border.all(color: const Color(0xFF007AFF), width: 0.5),
                ),
                child: Text(
...
class _NutrientBarChartPainter extends CustomPainter {
  ...
  void _drawGrid(
      Canvas canvas, double cl, double ct, double cw, double ch, double cb) {
    final lp = Paint()
  ...
  void _drawBubble(Canvas canvas, double cx, double cy, double value,
      Color color, bool isToxicity) {
    // Sombra
    canvas.drawCircle(
        Offset(cx + 1.5, cy + 2),
        19,
    ...
    _drawText(canvas, label,
        style: TextStyle(
            fontSize: 11,
            color:
                isToxicity ? const Color(0xFFE74C3C) : const Color(0xFF4A4A4A),
            fontWeight: FontWeight.w700),
        offset: Offset(cx - 19, cy - 7),
        width: 38,
  ...
  void _drawText(
    ...
    final builder =
        ui.ParagraphBuilder(ui.ParagraphStyle(textAlign: align, maxLines: 3))
  ...
  @override
  bool shouldRepaint(covariant _NutrientBarChartPainter old) =>
      old.data != data;
}
