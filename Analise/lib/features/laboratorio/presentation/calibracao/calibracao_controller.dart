...
class CalibracaoController extends StateNotifier<CalibracaoState> {
  ...
  Future<void> load() async {
    ...
    try {
      ...
      // ── Seed único do perfil padrão ──────────────────────────────────────
      // Executa APENAS quando o box nunca foi semeado (flag ausente) E não
      // há nenhum perfil local. Se o usuário deletar tudo depois, a flag
      // permanece true e o padrão NÃO é recriado.
  ...
  void atualizarProdutividade(double? value) {
    _updateDraft(state.draft.copyWith(produtividadeEsperadaTha: value));
  }
  ...
  void updateCalcario(CalcarioState novo) {
    state = state.copyWith(calcario: novo);
  ...
  void updateGesso(GessoState novo) {
    ...
    _syncDraftParametros('gesso', novo.parametros);
  }

  void updateFosforo(FosforoState novo) {
    state = state.copyWith(fosforo: novo);
  ...
  /// Extrator NÃO é alterado — depende do laboratório configurado por elemento.
  /// "Personalizada" não propaga NC — apenas atualiza o nome da referência.
  /// Regra: sobrescreve sempre — referência vence edições manuais anteriores.
  void propagarNcParaGrupo({
    required int grupoIndex,
    ...
    if (grupoIndex >= grupos.length) return;
    ...
    final membros = List<String>.from(
      (grupo['elementos'] as List<dynamic>?) ?? [],
    );

    for (final simbolo in membros) {
      if (!elementos.containsKey(simbolo)) continue;
    ...
    final draftAtualizado = state.draft.copyWith(parametrosCards: parametros);
    state = state.copyWith(
      draft: draftAtualizado,
      micros: MicronutrientesState(parametros: micros),
      errorMessage: null,
      successMessage: null,
    );
  }
  ...
  Future<bool> duplicarSelecionado() async {
    ...
    state = state.copyWith(
        selectedProfileId: null, successMessage: 'Perfil duplicado no rascunho.');
