...
- 2026-08-08: IPA 185 concluída com sucesso.
  ...
  - Observação: build gerado via `./build_ios.sh 185` na branch `cursor/fix-cliente-save-error-race-adad` (fix de race no erro de save do cliente + validação de nome + rules Firestore `cliente_tokens` já deployadas em `soloforte-106c8`); revisão pré-IPA: Aprovado com ressalvas; `product_modules_guard` aprovado; testes direcionados de clientes verdes; `Info.plist` embutido confirmou `CFBundleVersion=185`, `CFBundleShortVersionString=1.0.1`, bundle `com.soloforte.soloforte` e nome `Caderno de Solo`; `release_gate` reportou violação pré-existente de import cruzado em `lib/features/laboratorio/presentation/recomendacao/recomendacao_screen.dart` (`config_controller.dart`), mas a exportação concluiu; exportação via `tool/export_options_app_store.plist` com `manageAppVersionAndBuildNumber=false`

- 2026-09-11: IPA 186 concluída com sucesso.
  - Build: `1.0.1+186`
  - IPA: `Analise/build/ios/ipa/Caderno de Solo.ipa` (68 MB / 71,2 MB reportado pelo Flutter)
  - Observação: build gerado via `./build_ios.sh 186` na branch `cursor/fix-cliente-save-error-race-adad`, sobre os commits `465fe02` (catálogo de métodos de calagem + oculta tipos não usados no seletor), `1bc3e40` (método ⑧ CA+CD: NC = Y·Al³⁺ + (X − (Ca+Mg))) e `00df4eb` (limpeza de import + docs de auditoria), já enviados ao `origin`; revisão pré-commit Blocos A–F concluída e aprovada por Raudinei; `product_modules_guard` aprovado (clientes=24 dart files); `flutter analyze lib/` sem erros; `flutter test test/presentation/calibracao/` 95 verdes e `flutter test test/domain/` 275 verdes; `Info.plist` embutido confirmou `CFBundleVersion=186`, `CFBundleShortVersionString=1.0.1`, bundle `com.soloforte.soloforte` e nome `Caderno de Solo`; `release_gate` reportou a violação pré-existente de import cruzado em `lib/features/laboratorio/presentation/recomendacao/recomendacao_screen.dart` (`config_controller.dart`), mas a exportação concluiu; exportação via `tool/export_options_app_store.plist` com `manageAppVersionAndBuildNumber=false`
